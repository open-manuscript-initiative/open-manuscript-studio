...
/**
 * Selects one publication profile and applies its presentation defaults as one
 * atomic working-state change. Scholarly objects are not rewritten; only the
 * profile reference, presentation preferences and derived xref labels change.
 */
export function stagePublicationProfileChange(profileId: string): boolean {
  const profile = getPublicationProfile(profileId);
  ...
  useStudioStore.setState((state) => {
    ...
    const stateWithDefaults = applyPublicationProfileDefaults(
    ...
    const sameReference =
      previousReference?.id === nextReference.id &&
      previousReference?.version === nextReference.version;
    const sameSectionNumbering =
      (state.manuscript.sectionNumberingStyle ?? 'none') ===
      profile.rules.sections.numberingStyle;
    const sameCitationStyle =
      (state.manuscript.citationStyle ?? 'apa-7') ===
      profile.rules.citations.style;
    const sameObjectNumbering =
    ...
    if (!sameSectionNumbering) {
      events.push({
        operation: 'manuscript.sectionNumbering.set' as never,
        targetId: state.manuscript.id,
        path: '/sectionNumberingStyle',
        previousValue: state.manuscript.sectionNumberingStyle ?? 'none',
        nextValue: profile.rules.sections.numberingStyle,
      });
    }
    if (!sameCitationStyle) {
      events.push({
        operation: 'citation.style.set' as never,
        targetId: state.manuscript.id,
        path: '/citationStyle',
        previousValue: state.manuscript.citationStyle ?? 'apa-7',
        nextValue: profile.rules.citations.style,
      });
    }
    if (!sameObjectNumbering) {
      events.push({
        operation: 'crossReference.numbering.set' as never,
        targetId: state.manuscript.id,
        path: '/crossReferenceNumbering',
        previousValue: state.manuscript.crossReferenceNumbering ?? 'document',
        nextValue: profile.rules.objects.numbering,
      });
    }
...
function schedulePublicationProfileCheckpoint(): void {
  if (publicationProfileCheckpointTimer !== null) {
    clearTimeout(publicationProfileCheckpointTimer);
  }
