  ...
  const notice = [
    ...
    ...(assurance.reviewStatus === 'peer-reviewed'
      ? [
          ...
          ...(assurance.evidence.authority
            ? [
                `      data-omi-publication-venue-id="${escapeHtml(assurance.evidence.authority.venueId)}"`,
                `      data-omi-publication-venue-domain="${escapeHtml(assurance.evidence.authority.domain)}"`,
              ]
            : []),
        ]
      : []),
    ...
    ...(assurance.reviewStatus === 'peer-reviewed'
      ? [
          ...
          ...(assurance.evidence.authority
            ? [
                `        <small>${escapeHtml(copy.venue)} ${escapeHtml(assurance.evidence.authority.venueName)} · ${escapeHtml(assurance.evidence.authority.domain)}</small>`,
              ]
            : []),
        ]
      : []),
...
function disclosureCopy(
  locale: string,
  assurance: WebPublicationAssurance,
): { label: string; text: string; evidence: string; venue: string } {
  ...
  if (language === 'hu') {
    return {
      ...
      venue: 'Hitelesített folyóirati/kiadói autoritás:',
    };
  }
  if (language === 'de') {
    return {
      ...
      venue: 'Verifizierte Publikationsstellen-Autorität:',
    };
  }
  return {
    label: assurance.intent === 'public-interest'
    ...
    venue: 'Verified publication venue authority:',
  };
}
