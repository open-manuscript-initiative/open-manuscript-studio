...
export function PublicationProfilePanel() {
  ...
  function applyProfile(profile: OmiPublicationProfile): void {
    if (stagePublicationProfileChange(profile.id)) {
  ...
  return (
    <section className="publication-profile-view" aria-labelledby="publication-profile-title">
      ...
      <div className="publication-profile-experimental-note">
        {copy.experimental}
      </div>
      ...
      <section className="publication-profile-selector" aria-labelledby="publication-profile-choose">
        ...
        <div className="publication-profile-options">
          {BUILTIN_PUBLICATION_PROFILES.map((profile) => {
            ...
            return (
              <label
                className={`publication-profile-option${
                  isCandidate ? ' publication-profile-option--selected' : ''
                }`}
                key={profile.id}
              >
                <input
                ...
                <span className="publication-profile-option-copy">
                  <span className="publication-profile-option-title">
                    ...
                    {isActive ? (
                      <small className="publication-profile-active-badge">
                        {copy.active}
                      </small>
                    ) : null}
                  </span>
        ...
        <button
          type="button"
          className="studio-menu-primary-action publication-profile-apply"
          disabled={
            candidateProfile.id === activeProfile.id &&
            candidateProfile.version === activeProfile.version &&
            overrides.length === 0
          }
          onClick={() => applyProfile(candidateProfile)}
        >
        ...
        {statusMessage ? (
          <div className="publication-profile-status" role="status">
      ...
      <section className="publication-profile-readiness" aria-labelledby="publication-readiness-title">
        <div className="publication-profile-section-heading">
          <div>
            ...
            <p className={readiness.ready ? 'publication-ready' : 'publication-not-ready'}>
              {readiness.ready ? copy.ready : copy.notReady}
            </p>
          </div>
          <div className="publication-readiness-counts">
            <span>{readiness.errors} {copy.errors}</span>
            <span>{readiness.warnings} {copy.warnings}</span>
          </div>
        </div>

        {issues.length === 0 ? (
          <p className="publication-profile-no-issues">
              ...
              >
                ...
                <span>
                  ...
                  {issue.code === 'too-few-keywords' && issue.detail
                    ? ` (${issue.detail})`
                    : ''}
                  {issue.code === 'profile-override' && issue.detail
                    ? ` — ${issue.detail}`
                    : ''}
                </span>
              </li>
      ...
      <section className="publication-profile-export">
        <div>
          <strong>{copy.exportProfile}</strong>
...
function ProfileRuleSummary({
  ...
  return (
    <section className="publication-profile-rules" aria-labelledby="publication-profile-rules-title">
      <div className="publication-profile-section-heading">
        <div>
          <h4 id="publication-profile-rules-title">{copy.rules}</h4>
          <p>
            <strong>{localized.name}</strong> · {copy.profileVersion} {profile.version}
            {profile.publisher ? ` · ${copy.profilePublisher}: ${profile.publisher}` : ''}
          </p>
        </div>
      </div>
      ...
      <div className="publication-profile-rule-grid">
        ...
        <RuleCard title={copy.sections}>
          <RuleLine label={copy.numbering} value={rules.sections.numberingStyle} />
          <RuleLine label={copy.maxDepth} value={String(rules.sections.maxNumberedDepth)} />
        </RuleCard>

        <RuleCard title={copy.citations}>
          <RuleLine label={copy.citationStyle} value={rules.citations.style} />
          <RuleLine label={copy.notePlacement} value={notePlacementLabel(rules.notes.placement, copy)} />
        </RuleCard>

        <RuleCard title={copy.objects}>
          <RuleLine label={copy.objectNumbering} value={rules.objects.numbering === 'section' ? copy.section : copy.document} />
          <RuleLine label={copy.figureCaption} value={rules.objects.figureCaptionPosition === 'above' ? copy.above : copy.below} />
          <RuleLine label={copy.tableCaption} value={rules.objects.tableCaptionPosition === 'above' ? copy.above : copy.below} />
        </RuleCard>

        <RuleCard title={copy.contributors}>
          <RuleLine label={copy.showAffiliations} value={rules.contributors.showAffiliations ? copy.yes : copy.no} />
          <RuleLine label={copy.showOrcid} value={rules.contributors.showOrcid ? copy.yes : copy.no} />
        </RuleCard>

        <RuleCard title={copy.requirements}>
          <RuleLine label="Abstract" value={rules.metadata.requireAbstract ? copy.requirementLabels.required : copy.requirementLabels.off} />
          <RuleLine label="Keywords" value={rules.metadata.minimumKeywords > 0 ? `≥ ${rules.metadata.minimumKeywords}` : copy.requirementLabels.off} />
          <RuleLine label="Affiliation" value={copy.requirementLabels[rules.metadata.affiliation]} />
          <RuleLine label="ORCID" value={copy.requirementLabels[rules.metadata.orcid]} />
        </RuleCard>
      </div>

      <div className="publication-profile-output-row">
        <strong>{copy.outputs}</strong>
        <div className="publication-profile-output-list">
          {rules.outputs.map((format) => (
            <code key={format}>{format.toUpperCase()}</code>
          ))}
        </div>
      </div>
    </section>
...
function RuleCard({
  ...
  return (
    <div className="publication-profile-rule-card">
      <h5>{title}</h5>
      <dl>{children}</dl>
    </div>
  );
...
function notePlacementLabel(
  placement: OmiPublicationProfile['rules']['notes']['placement'],
  copy: ReturnType<typeof getPublicationProfileCopy>,
): string {
