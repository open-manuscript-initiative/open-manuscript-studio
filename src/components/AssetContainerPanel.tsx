    ...
    <section className="omi-container-card" aria-labelledby="omi-container-title">
      ...
      <section className="omi-container-import" aria-labelledby="omi-container-import-title">
        <div className="omi-container-header">
          ...
          <div>
            <h4 id="omi-container-import-title">{copy.importTitle}</h4>
        ...
        <input
          ...
          type="file"
          accept=".omi,application/vnd.openmanuscript.omi+zip,application/zip"
          onChange={(event) => void inspectPackage(event)}
        />
        ...
        <div className="omi-container-actions">
          ...
          >
            ...
            {busy === 'inspect' ? copy.inspecting : copy.choosePackage}
          </button>
        </div>

        {importedTitle ? (
          <p className="omi-container-status omi-container-status--ok" role="status">
            <CheckCircle2 size={15} aria-hidden="true" />
            {copy.imported} {importedTitle ? `— ${importedTitle}` : ''}
          </p>
        ) : null}

        {importReadError ? (
          <p className="omi-container-status omi-container-status--error" role="alert">
            {importReadError}
          </p>
        ) : null}
        ...
        {importPlan ? (
          <div className="omi-container-import-report">
            ...
            <div className="omi-container-import-summary">
              <span><strong>{importPlan.summary.entryCount}</strong> {copy.entries}</span>
              <span><strong>{importPlan.summary.assetCount}</strong> {copy.packageAssets}</span>
              <span><strong>{importPlan.summary.revisionCount}</strong> {copy.revisions}</span>
              <span>
                <strong>{importPlan.summary.verifiedChecksums}/{importPlan.summary.declaredChecksums}</strong>{' '}
                {copy.checksums}
              </span>
            </div>
            ...
            {importPlan.summary.manuscriptId ? (
              <dl className="omi-container-identities">
                <div>
                  <dt>{copy.manuscriptId}</dt>
                  <dd><code>{importPlan.summary.manuscriptId}</code></dd>
                </div>
                <div>
                  <dt>{copy.headRevision}</dt>
                  <dd><code>{importPlan.summary.headRevisionId}</code></dd>
                </div>
              </dl>
            ) : null}
            ...
            <div className="omi-container-actions">
              ...
              >
                ...
                {busy === 'import' ? copy.importing : copy.importPackage}
              </button>
            </div>
          </div>
        ) : null}
...
function ContainerDiagnostics({
  ...
  return (
    <details className="omi-container-diagnostics" open={errors > 0}>
      <summary>
        {title} · {errors} / {warnings}
      </summary>
      <p className={errors ? 'omi-container-status omi-container-status--error' : 'omi-container-status omi-container-status--ok'}>
        {errors ? blocked : ready}
      </p>
      <ul>
