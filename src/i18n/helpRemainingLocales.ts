...
export function getRemainingFullHelpCopy(locale:RemainingFullHelpLocale):HelpCopy{const source=getAdditionalHelpCopy(locale);if(!source)throw new Error(`Missing localized help source for ${locale}`);const enriched=enrichAdditionalHelp(locale,source);if(locale==='hr'||locale==='lt'||locale==='lv')return applyHrLtLvHelpQuality(locale,enriched);return enriched;}
