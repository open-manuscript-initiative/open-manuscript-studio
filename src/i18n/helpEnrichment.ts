...
/**
 * Adds a fully localized practical guidance layer to the compact help copies.
 * The topic bodies themselves remain in helpAdditional.ts; this module makes
 * every supported additional locale expose the same help structure as the
 * detailed HU/EN/DE/IT copies (body + practical guidance for every topic).
 */
...
export function enrichAdditionalHelp(locale: string, copy: HelpCopy): HelpCopy {
  ...
  // The compact translations predate author signatures. Keep their existing
  // twenty topics intact and add the current signing workflow explicitly.
