...
for (const entry of await fs.readdir(poRoot, { withFileTypes: true })) {
  ...
  try {
    ...
    // Ignore pending/completion working directories.
  }
}
localeDirs.sort();
...
async function readEntries(locale) {
  return parsePo(await fs.readFile(path.join(poRoot, locale, 'studio.po'), 'utf8'));
}
...
for (const locale of localeDirs) {
  ...
  for (const [pointer, referenceEntry] of referenceByPointer) {
    ...
    if (
      ...
      resolved.value === referenceEntry.source &&
      !resolved.reviewedByOverlay &&
  ...
  const percent = expectedPointers.size === 0 ? 100 : (translated / expectedPointers.size) * 100;
...
console.log('PO + reviewed-overlay translation audit');
console.log('=========================================');
console.log(`Reference locale: ${referenceLocale}`);
console.log(`Expected entries: ${expectedPointers.size}`);
console.log('');
console.log('Locale  Progress   Translated  Identical  Overlay  Empty  Structural  Status');
console.log('------  ---------  ----------  ---------  -------  -----  ----------  ------');
for (const row of rows) {
  const structural = row.missing + row.extra + row.stale + row.duplicates;
  const status = row.complete ? 'complete' : 'in progress';
  console.log(`${row.locale.padEnd(6)}  ${row.percent.toFixed(1).padStart(7)}%  ${String(row.translated).padStart(10)}  ${String(row.identical).padStart(9)}  ${String(row.reviewedOverlay).padStart(7)}  ${String(row.empty).padStart(5)}  ${String(structural).padStart(10)}  ${status}`);
}

const nonReferenceRows = rows.filter((row) => row.locale !== referenceLocale);
console.log('');
console.log(`Identical-to-English candidates: ${nonReferenceRows.reduce((sum,row)=>sum+row.identical,0)}`);
console.log(`Reviewed overlay values applied: ${nonReferenceRows.reduce((sum,row)=>sum+row.reviewedOverlay,0)}`);
...
if (completenessFailures > 0) {
  const failed = rows.filter((row)=>row.complete&&row.identical>0).map((row)=>`${row.locale} (${row.identical})`).join(', ');
  throw new Error(`Complete locale(s) still contain unreviewed English-identical candidates: ${failed}`);
}
