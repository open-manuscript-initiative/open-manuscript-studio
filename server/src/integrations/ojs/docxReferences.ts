...
/**
 * Restores semantic external links, DOI links and Word bibliography/citation
 * fields after the lower-level DOCX enrichment passes have completed.
 *
 * This pass deliberately runs last: it splits already-styled text runs at
 * semantic link/citation boundaries without discarding bold/italic/language
 * metadata or note anchors.
 */
export function applyReferenceSemantics(
  ...
  const relationships = relsXml ? parseRelationships(relsXml) : new Map<string, Relationship>();
...
function parseParagraphs(
  ...
  while ((match = pattern.exec(body))) {
    ...
    while ((hyperlink = hyperlinkPattern.exec(paragraphXml))) {
      ...
      spans.push({ start, end: start + label.length, href: normalizeHref(relationship.target) });
    }

    spans.push(...fieldSpans(paragraphXml));
    spans.push(...doiSpans(text));

    if (text || spans.length) result.push({ text: text.trim(), spans: deduplicateSpans(spans) });
  }
...
function fieldSpans(paragraphXml: string): Span[] {
  ...
  // Simple fields are common in generated DOCX files and are unambiguous.
  ...
  while ((match = simple.exec(paragraphXml))) {
    ...
    if (citationTags.length) {
      spans.push({
        ...
        citation: { kind: 'citationReference', sourceTags: citationTags, label: label || `[${citationTags.join('; ')}]` },
      });
      continue;
    }
    const href = parseHyperlinkInstruction(instruction);
    if (href && label) spans.push({ start, end: start + label.length, href: normalizeHref(href) });
  }

  // Complex fields: BEGIN -> instrText -> SEPARATE -> visible result -> END.
...
function applySpans(paragraph: OjsDocxParagraph, spans: readonly Span[]): OjsDocxParagraph {
  ...
  for (const item of original) {
    ...
    while (local < textItem.text.length) {
      while (spanIndex < sorted.length && (sorted[spanIndex]?.end ?? 0) <= absolute + local) spanIndex += 1;
      ...
      if (span.citation && span.start === absolute + local) {
        const consume = Math.max(0, Math.min(textItem.text.length - local, span.end - span.start));
      ...
      if (span.href && span.start <= absolute + local && span.end > absolute + local) {
        const take = Math.min(textItem.text.length - local, span.end - (absolute + local));
        result.push({ ...textItem, text: textItem.text.slice(local, local + take), href: span.href });
        local += take;
        if (absolute + local >= span.end) spanIndex += 1;
        continue;
      }
...
function parseBibliography(buffer: Buffer): OjsSourceBibliographicRecord[] {
  ...
  for (const name of centralDirectory(buffer).keys()) {
    ...
    while ((source = sourcePattern.exec(xml))) {
      ...
      if (standard) identifiers.push({ scheme: /^97[89]/.test(standard.replace(/[-\s]/g, '')) ? 'isbn' : 'identifier', value: standard });
      const doi = extractDoi([url, standard, firstElementText(body, 'Comments')].filter(Boolean).join(' '));
      ...
      records.push({
        ...
        containerTitle: firstElementText(body, 'JournalName')?.trim(),
        issued: firstElementText(body, 'Year')?.trim(),
        publisher: firstElementText(body, 'Publisher')?.trim(),
        place: firstElementText(body, 'City')?.trim(),
        volume: firstElementText(body, 'Volume')?.trim(),
        issue: firstElementText(body, 'Issue')?.trim(),
        pages: firstElementText(body, 'Pages')?.trim(),
        identifiers,
        url,
      });
    }
...
function parseContributors(xml: string): OjsSourceBibliographicContributor[] {
  ...
  while ((match = personPattern.exec(xml))) {
    const body = match[1] ?? '';
    const givenName = [firstElementText(body, 'First'), firstElementText(body, 'Middle')].filter(Boolean).join(' ').trim();
    const familyName = firstElementText(body, 'Last')?.trim();
    if (!givenName && !familyName) continue;
    result.push({ role: 'author', ...(givenName ? { givenName } : {}), ...(familyName ? { familyName } : {}) });
  }
  return result;
}
...
function parseRelationships(xml: string): Map<string, Relationship> {
  ...
  while ((match = pattern.exec(xml))) {
    ...
    result.set(id, { target, external: attr(attributes, 'TargetMode')?.toLowerCase() === 'external' });
  }
  return result;
}
...
function parseCitationTags(value: string): string[] {
  ...
  const remainder = input.slice(input.search(/\bCITATION\b/i)).replace(/^CITATION\s+/i, '');
  const tokens = remainder.match(/"[^"]*"|'[^']*'|\S+/g) ?? [];
  const tags: string[] = [];
  let expectSource = true;
  for (const token of tokens) {
    if (/^\\m$/i.test(token)) { expectSource = true; continue; }
    if (token.startsWith('\\')) { expectSource = false; continue; }
...
function extractDoi(value: string): string | undefined {
  return /10\.\d{4,9}\/[\w.()/:;-]+/i.exec(value)?.[0]?.replace(/[.,;:]+$/, '');
}
...
function descendantsText(xml: string, name: string): string[] {
  const result: string[] = [];
  const pattern = new RegExp(`<(?:[A-Za-z_][\\w.-]*:)?${name}(?:\\s[^>]*)?>([\\s\\S]*?)<\\/(?:[A-Za-z_][\\w.-]*:)?${name}>`, 'gi');
  let match: RegExpExecArray | null;
  while ((match = pattern.exec(xml))) result.push(decodeXml(match[1] ?? ''));
  return result;
}
...
function readElementBody(xml: string, name: string): string | undefined {
  return new RegExp(`<(?:[A-Za-z_][\\w.-]*:)?${name}\\b[^>]*>([\\s\\S]*?)<\\/(?:[A-Za-z_][\\w.-]*:)?${name}>`, 'i').exec(xml)?.[1];
}

function attr(attributes: string, name: string): string | undefined {
  return new RegExp(`(?:^|\\s)(?:[A-Za-z_][\\w.-]*:)?${name}\\s*=\\s*["']([^"']+)["']`, 'i').exec(attributes)?.[1];
}
